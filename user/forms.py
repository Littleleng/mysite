...
class ChangePasswordForm(forms.Form):
    ...
    def clean_old_password(self):
        #验证旧密码
        old_password = self.cleand_data.get('old_password','')
...
class ForgotPasswordForm(forms.Form):
    ...
    def clean_email(self):
        email = self.cleaned_data['email'].strip()#Python strip() 方法用于移除字符串头尾指定的字符（默认为空格或换行符）或字符序列。
        if not User.objects.filter(email=email).exists():#objects.filter支持更强大的查询功能,.exists() 如果目录不存在，会返回一个0值
            raise forms.ValidationError('邮箱不存在')#raise 抛出异常,ValidationError告诉错误信息
        return email
    ...
    def clean_verification_code(self):
        ...
        if verification_code == '':
            raise forms.ValidationError('验证码不能为空')
        # 判断验证码
        code = self.request.session.get('forgot_password_code', '')#如果其中session包含具有值的键，则"name"它返回与该键关联的值（可能是False），否则（如果没有名为“ name”的键），则返回False。
        verification_code = self.cleaned_data.get('verification_code', '')#.get()基本上是从字典中获取元素的捷径。我通常.get()在不确定字典中的条目是否存在时使用。
        if not (code != '' and code == verification_code):
            raise forms.ValidationError('验证码不正确')
        return verification_code
